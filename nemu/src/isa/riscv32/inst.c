...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(Mr(src1 + imm, 2),16));/* sign-extends */
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = Mr(src1 + imm, 2));/* zero-extends */
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = SEXT(Mr(src1 + imm, 1),8));/* sign-extends */
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = SEXT(Mr(src1 + imm, 4),32));/* sign-extends */
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (sword_t)src1 >> (imm & 0x1f));/* arithmetic right shift */
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> (imm & 0x1f));/* logical right shift */
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << (imm & 0x1f));/* logical left shift */
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2);
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << src2);
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(dest) = src1 >> (src2 & 0x1f)); /* logical shift */
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = SEXT(src1,32) >> (src2 & 0x1f)); /* arithmetic shift */
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) =(long long)(sword_t)src1 * (long long)(sword_t)src2 >> 32); /* be careful of the 32 bit here */
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->snpc, s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, ftrace_print(s->pc, s->dnpc, s->isa.inst.val)););
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->snpc, s->dnpc = src1 + imm; IFDEF(CONFIG_FTRACE, ftrace_print(s->pc, s->dnpc, s->isa.inst.val)););
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, ECALL(s->dnpc)); //isa_reg_display(NULL););//IFDEF(CONFIG_DIFFTEST,difftest_skip_ref()););
